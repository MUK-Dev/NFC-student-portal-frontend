...
import { generateAnySubjectReportRequest } from '../Services/API/generateAnySubjectReportRequest'

import NFCLogo from '../Assets/Images/NFC Iet Logo.png'

import useAuth from './useAuth'

export default function useAdminGeneratePDFReport() {
  const [isGenerating, setIsGenerating] = useState(false)
  const [error, setError] = useState(null)
  const { token } = useAuth()

  const generatePDF = async (teacherId, subjectId, sectionId) => {
    setIsGenerating(prev => true)
    setError(null)
    try {
      const data = await generateAnySubjectReportRequest(
        token,
        teacherId,
        subjectId,
        sectionId,
